        ...
        for node in Self::instantiate(expr).as_ref().iter() {
            match node {
                ...
                lang::VecLang::Vec(x) => {
                    ...
                    for val in &**x {
                        works &= { 
                            match &types[usize::from(*val)] {
                                Some(VecLangType::Vector) => false,
                                Some(VecLangType::Scalar) => true,
                                Some(VecLangType::Variable) => true,
                                // jb: case of none -> unsure about this
                                _ => false
                            } 
                        }
                    }
                ...
                // constants
                lang::VecLang::List(_) | lang::VecLang::LitVec(_) => {
                    types.push(Some(VecLangType::Vector))
                }
