...
pub fn build_litvec_rule_desugared(vec_width: usize) -> Vec<egg::Rewrite<VecLangDesugared, ()>> {
    ...
    let searcher: egg::Pattern<VecLangDesugared> =
        format!("(Vec {})", all_gets).parse().unwrap();
    let applier: egg::Pattern<VecLangDesugared> =
